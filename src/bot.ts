...
export class ChatGPTBot {
  ...
  async trySay(
    ...
    let message = mesasge;
    while (message.length > config.singleMessageMaxSize) {
      messages.push(message.slice(0, config.singleMessageMaxSize));
      message = message.slice(config.singleMessageMaxSize);
    }
